...
namespace EmoAns
{
  ...
  int weight;//word weight,read weight
    ...
    class WordParsing
    {
        ...
        /// </summary>
        /// 
        public WordParsing() //万恶的初始化dll文件
        {
        ...
        public int Counting(string source) //计算词源有多少个词
        {
            try
            {
                int count = NLPIR_GetParagraphProcessAWordCount(source);
                return count;
            }
            catch (Exception e)
            { return 0; }
        ...
        public string Parsing(string source, int isTag)
        {
            try
            {
                 
                IntPtr intPtr = NLPIR_ParagraphProcess(source, isTag);//切分结果保存为IntPtr类型
                System.Console.WriteLine("切分结果保存为IntPtr类型 success!");
                String str = Marshal.PtrToStringAnsi(intPtr);//将切分结果转换为string
                System.Console.WriteLine("将切分结果转换为 success!");
